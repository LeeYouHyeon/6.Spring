...
public class FileSweeper {
	...
	public void fileSweeper() {
		...
		log.info(">>> al file Object >> {}", allFileObject.toString());
		
		// 실제 저장된 파일 목록과 DB에 존재하는 파일을 비교하여 DB에 없는 파일을 삭제
		for (File file : allFileObject) {
			String storedFileName = file.toPath().toString();
			if (currFiles.contains(storedFileName)) continue;
			
			file.delete(); // 존재하지 않는 파일 삭제
